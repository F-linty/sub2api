...
func TestExtractOpenAIUsageFromJSONBytes_AcceptsResponseAndChatUsageShapes(t *testing.T) {
	...
	usage, ok = extractOpenAIUsageFromJSONBytes([]byte(`{"usage":{"input_tokens":20,"output_tokens":8,"output_tokens_details":{"reasoning_tokens":6}}}`))
	require.True(t, ok)
	require.Equal(t, 6, usage.ReasoningTokens)

	usage, ok = extractOpenAIUsageFromJSONBytes([]byte(`{"usage":{"prompt_tokens":20,"completion_tokens":8,"completion_tokens_details":{"reasoning_tokens":5}}}`))
	require.True(t, ok)
