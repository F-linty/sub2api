-- CockroachDB variant of 131. CRDB does not support to_char(decimal, string).
-- Use string casting instead to normalize the rebate rate value.
UPDATE settings
SET value = trim(trailing '0' FROM trim(trailing '.' FROM ((value::numeric * 100)::decimal(28,8))::string)),
    updated_at = NOW()
