-- CockroachDB variant of 108. Two changes vs the PostgreSQL original:
--   1. The signup_source CHECK guard (DO/PL-pgSQL) is flattened to plain ADD CONSTRAINT.
--   2. Per-statement autocommit (the runner splits this overlay) lets the ADD COLUMN
--      backfill commit before the UPDATE and the CHECK, avoiding CRDB's
--      "column is being backfilled" async-schema-change error.
-- Table/index DDL below is unchanged from the original.
ALTER TABLE users
