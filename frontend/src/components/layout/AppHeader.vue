<template>
  <header class="sticky top-0 z-30 border-b border-transparent bg-white/95 dark:border-dark-800 dark:bg-dark-950/95">
    <div class="flex h-[72px] items-center justify-between px-5 md:px-8 lg:px-12">
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="hidden lg:block">
          <h1 class="text-[1.7rem] font-semibold leading-tight text-[#151515] dark:text-white">
            {{ pageTitle }}
          </h1>
          <p v-if="pageDescription" class="mt-0.5 text-xs text-gray-500 dark:text-dark-400">
            {{ pageDescription }}
          </p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <a
          ...
          class="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm font-medium text-gray-600 transition-colors hover:bg-primary-50 hover:text-gray-900 dark:text-dark-400 dark:hover:bg-dark-800 dark:hover:text-white"
        >
        ...
        <div
          v-if="user"
          class="group relative hidden items-center gap-2 rounded-xl border border-[#eadfd9] bg-[#fff8f3] px-3 py-1.5 text-[#9f4232] shadow-[0_8px_22px_rgba(74,39,29,0.05)] transition-colors hover:border-[#dfccc3] hover:bg-[#fff3eb] dark:border-dark-700 dark:bg-dark-900/80 dark:text-red-200 sm:flex"
        >
          <svg
            class="h-4 w-4 text-[#b6533f] dark:text-red-200"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            stroke-width="1.5"
          ...
          <span class="text-sm font-semibold text-[#9f4232] dark:text-red-100">
        ...
        <div v-if="user" class="relative" ref="dropdownRef">
          ...
          >
            <div class="flex h-8 w-8 items-center justify-center overflow-hidden rounded-md bg-slate-950 text-sm font-medium text-white shadow-sm dark:bg-white dark:text-slate-950">
              <img
                v-if="avatarUrl"
                :src="avatarUrl"
                :alt="displayName"
          ...
          <transition name="dropdown">
            <div v-if="dropdownOpen" class="dropdown right-0 mt-2 w-56">
              ...
              <div class="text-xs text-gray-500 dark:text-dark-400">
                ...
                <div class="text-sm font-semibold text-[#9f4232] dark:text-red-100">
                  {{ formatHeaderMoney(availableBalance) }}
                </div>
